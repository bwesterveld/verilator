...
void EmitCImp::emitInt(AstNodeModule* modp) {
    ...
    /*
     * This fi_object wraps around CData and only CData datatypes.
     * All control signals fit in the CData type. The assignment operator
     * is overloaded to check whether a fault should be injected in that
     * signal at the time of the assignment. */
    if (v3Global.opt.fault_injection()){ //fi
        puts("class fi_object {\n");
        puts("  public:\n");
        puts("    static std::string injection_location;\n");
        puts("    static QData injection_time;\n");
        puts("    static QData release_time;\n");
        puts("    static CData faulty_value;\n");
        puts("\n");
        puts("    fi_object(const char* name) : m_value(CData(0)), m_name(name) { }\n");
        puts("    fi_object() : m_value(CData(0)), m_name(\"\") { }\n");
        puts("    ~fi_object() {}\n");
        puts("\n");
        puts("    std::string name() {\n");
        puts("      return m_name;\n");
        puts("    }\n");
        puts("\n");
        ...
        puts("\n");
        puts("    inline fi_object& operator=(const CData a) {\n");
        puts("      m_value = a;\n");
        puts("\n");
        puts("      if (m_name == fi_object::injection_location) {\n");
        puts("        if (fi_object::injection_time != 0 && fi_object::release_time != 0) {\n");
        ...
        puts("              std::cout << \"Injecting stuck-at-0 fault at signal\" << m_name << \" during [\" << fi_object::injection_time << \",\" << fi_object::release_time << \"]\";\n");
        puts("              runOnce = false;\n");
        puts("            }\n");
        puts("          }\n");
        puts("        }\n");
